...
export async function UserNav() {
  ...
  return (
    <div>
      <DropdownMenu>
        <DropdownMenuTrigger>
          <div className="rounded-full border p-2 lg:px-4 lg:py-2 hidden sm:flex items-center gap-x-2 hover:shadow-md  transition-all">
            <Menu size={24} />
            <img
              src={
                user?.picture ??
            ...
            <React.Fragment>
              <DropdownMenuItem>
                <form className="w-full" action={createHomeWithUserId}>
                  <button className="w-full font-medium text-start">
                    {getHome() === null ? "Add your home" : "Airbnb your home"}
                  </button>
                </form>
              </DropdownMenuItem>
              <DropdownMenuItem>
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem>
                <LogoutLink className="flex gap-x-[0.3rem] items-center justify-center">
                  Log out
                  <IoIosLogOut />
                </LogoutLink>
              </DropdownMenuItem>
